...
- **Images get aggressively rewritten; PDFs do not.** `imageToPDF` (main.go:192) downscales to `maxLongEdge=2000px`, converts to 8-bit grayscale, and re-encodes JPEG at quality 80 before placing on A4. Existing PDF inputs are passed through untouched — `api.OptimizeFile` only deduplicates objects, it does **not** recompress embedded images. If a user complains about a large output, the culprit is almost always a fat input PDF, not the image pipeline.
